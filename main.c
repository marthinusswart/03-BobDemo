...
const UWORD copper2[] __attribute__((section(".MEMF_CHIP"))) = {
	...
	0x4f01, 0xff00, offsetof(struct Custom, color[0]), 0x0aaa, // line 0x4e
...
int main()
{
	...
	debug_register_bitmap(bob2, "amiga-bob.bpl", 64, 64, 5, debug_resource_bitmap_interleaved | debug_resource_bitmap_masked);
	...
	*copPtr++ = 0x7fff;
	...
	while (!MouseLeft())
	{
		...
		int speed = 1; // move every speed frames
		if (f % fps == 0)
		{
			// clear
			WaitBlit();
